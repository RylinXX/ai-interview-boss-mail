...
def upgrade() -> None:
    position_urgency_enum = postgresql.ENUM('LOW', 'MEDIUM', 'HIGH', 'URGENT', name='positionurgency', create_type=False)
    position_type_enum = postgresql.ENUM('FULL_TIME', 'PART_TIME', 'CONTRACT', 'INTERNSHIP', name='positiontype', create_type=False)
    position_urgency_enum.create(op.get_bind(), checkfirst=True)
    position_type_enum.create(op.get_bind(), checkfirst=True)

    op.add_column('positions', sa.Column('urgency', position_urgency_enum, server_default='MEDIUM', nullable=True))
    op.add_column('positions', sa.Column('position_type', position_type_enum, server_default='FULL_TIME', nullable=True))
...
def downgrade() -> None:
    ...
    position_urgency_enum.drop(op.get_bind(), checkfirst=True)
