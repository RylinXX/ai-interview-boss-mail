...
def test_read_file_content_prefers_local_pdf_text_before_model_document_extraction(monkeypatch, tmp_path):
    pdf = tmp_path / "resume.pdf"
    pdf.write_bytes(b"%PDF-1.4")

    monkeypatch.setattr(resume_service, "_extract_pdf_text", lambda file_path: "本地完整解析文本")
    monkeypatch.setattr(
        resume_service,
        "extract_resume_text_from_document",
        lambda file_path: (_ for _ in ()).throw(AssertionError("model extraction should not run")),
    )

    assert resume_service.read_file_content(str(pdf)) == "本地完整解析文本"
